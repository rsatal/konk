#!/bin/sh
JWT="eyJhbGciOiJSUzUxMiIsInR5cCI6IkpXVCJ9..ZOx1TlV2-CDGhCyOrd7UoZQ_UrKgYgfxejgITuxnqOU8yuI5g5_nVudoqMUjwBMKxTLAdBGknf5_8CKWg2wMnvvvT06lxaqbQGurh3GU0nxnSi83x2_0CGXiEWr5PxvwZidrJ1s8eJLuzpMolQRaz1t_AuA991eo7s8apX6rqcIhfpuh8OYzT4OFhtaUMkk9FHczWj8UsALAk0F2nRLokvf0M5Wz5cLHGKN5m34JKrDKMrDdGNsxkCqwXB4-l5KBsWvW4ikRyFQY7d6BabIGJZhi4G2pDvkNnbAcCHA--gxKyQ-xpSAniLGH9BX8niKmJBQ-KHtrdR4ZHvkTbvJG5A"

echo "=== 1. API Discovery (no auth needed) ==="
echo "--- kubectl get --raw /apis/tagging.bulk.infoblox.com/v1alpha1 ---"
kubectl --kubeconfig /etc/kubernetes/admin.conf get --raw /apis/tagging.bulk.infoblox.com/v1alpha1
echo ""

echo ""
echo "=== 2. List tags via curl with JWT through konk ==="
echo "--- curl https://bulk-konk.aggregate.svc:6443/apis/tagging.bulk.infoblox.com/v1alpha1/namespaces/default/tags ---"
curl -sk \
  --cert /etc/kubernetes/tls.crt \
  --key /etc/kubernetes/tls.key \
  --cacert /etc/kubernetes/ca.crt \
  -H "Authorization: Bearer ${JWT}" \
  "https://bulk-konk.aggregate.svc:6443/apis/tagging.bulk.infoblox.com/v1alpha1/namespaces/default/tags" 2>&1
echo ""

echo ""
echo "=== 3. List tags via direct REST to tagging-v2 with JWT ==="
echo "--- curl http://tagging.tagging-v2.svc.cluster.local:8081/v2/tags?_limit=5 ---"
curl -sk \
  -H "Authorization: Bearer ${JWT}" \
  "http://tagging.tagging-v2.svc.cluster.local:8081/v2/tags?_limit=5" 2>&1
echo ""
